...
**Decision / learning.** Kept - this is the load-bearing iteration, and we ran the control that proves the attribution: v1 (scanner report and file tools, but no vision) was also alt-judged and still fabricated - mean 1.63 with **3 hallucinated alts**. So the progression is baseline 4+4 fabrications, v1 3, v3/final 0. The scanner report alone does not stop fabrication; mandatory perception does. Grounding is an architecture property, not a model property.
...
**Decision / learning.** Kept - and, after the judge-agent review of this submission asked "does the machinery ever actually fire?", we proved it two ways. First we tried to provoke a natural failure with a much smaller model (`claude-haiku-4-5`): it *also* cleared the corpus cleanly - capability saturation goes deeper than we thought. So we added a fault-injection mode (`STEPFREE_CHAOS=1`) that sabotages one fixer round with the exact damage class the research documents (deleting an image and a paragraph). On record, in `runs/fault-injection-chaos` and `trajectories/`: the integrity gate **FAILED the round (text-loss, image-loss)**, the retry recovered the image but not the text, and the orchestrator **rolled the round back and routed it to human review** while the rest of the pipeline completed normally. The gates are quiet in normal runs because the fixer is good - not because they are decorative.
...
Review lane on the challenge fixtures: the reviewer found **all 5 scanner-invisible traps we planted** - the image-of-text hero (WCAG 1.4.5), the keyboard-dead `<div onclick>` dropdown, the `<span onclick>` fake button, and both fake `<div>` buttons on the clinic - each flagged high-confidence with a copy-pasteable proposed fix, plus legitimate bonus findings we did not plant (missing `autocomplete` for WCAG 1.3.5, a Donate button pointing at the wrong anchor). 5/5 trap recall (the traps are enumerated with ids in the two challenge fixtures' meta.json).
