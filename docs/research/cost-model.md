...
On our corpus the one-shot did not damage pages - but it did nondeterministically silence and fabricate image content across runs (see EVALUATION.md, Variance), which a responsible owner can only catch by reviewing the output page by page; that is the labor being priced here.
